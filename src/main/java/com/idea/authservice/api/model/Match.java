package com.idea.authservice.api.model;

import com.idea.authservice.api.model.enums.MatchStatus;
import com.idea.authservice.api.patterns.observer.MatchObserver;
import com.idea.authservice.api.patterns.observer.MatchSubject;
import com.idea.authservice.api.patterns.state.CreatedState;
import com.idea.authservice.api.patterns.state.MatchState;
import com.idea.authservice.api.patterns.strategy.join.JoinStrategy;
import com.idea.authservice.auth.domain.model.Player;
import jakarta.persistence.*;
import lombok.AllArgsConstructor;
import lombok.Data;
import lombok.EqualsAndHashCode;
import lombok.NoArgsConstructor;
import lombok.extern.slf4j.Slf4j;
import lombok.ToString;
import com.fasterxml.jackson.annotation.JsonIgnore;

import java.time.LocalDateTime;
import java.util.ArrayList;
import java.util.List;
import java.util.UUID;

@Entity
@Table(name="matches")
@Inheritance(strategy = InheritanceType.SINGLE_TABLE)
@DiscriminatorColumn(name = "match_type")
@Data
@EqualsAndHashCode(onlyExplicitlyIncluded = true)
@AllArgsConstructor
@NoArgsConstructor
@Slf4j
public class Match implements MatchSubject {
    @Id
    @GeneratedValue(strategy = GenerationType.IDENTITY)
    @EqualsAndHashCode.Include
    private Long id;
    private LocalDateTime dateTime;
    private String location;
    private String region;
    private UUID creatorId;
    @Transient
    @JsonIgnore
    @ToString.Exclude
    private MatchState state;
    private String level;
    private String msg;
    private String sport;
    @Enumerated(EnumType.STRING)
    @Column(nullable = false)
    private MatchStatus status;

    @Column(name = "required_players")
    private Integer requiredPlayers;

    @Column(name = "duration_minutes")
    private Integer durationMinutes;
    //estrategia de union configurada  skill, location null = libre
    @Column(name = "join_strategy_key")
    private String joinStrategyKey;
    @ManyToMany
    @JoinTable(name = "match_players",
            joinColumns = @JoinColumn(name = "match_id"),
            inverseJoinColumns = @JoinColumn(name = "player_id"))
    private java.util.Set<Player> players = new java.util.HashSet<>();
    @Transient
    @ToString.Exclude
    private final List<MatchObserver> observers = new ArrayList<>();

    @Transient
    @JsonIgnore
    @ToString.Exclude
    private JoinStrategy joinStrategy;

    public void setState(MatchState state) {
        this.state = state;
        notifyObservers("El partido cambió de estado a " + state.getClass().getSimpleName());
    }

    public void start() {
        this.msg = state.start(this);

    }

    public void finish() {
        this.msg = state.finish(this);
    }

    public void cancel() {
        this.msg = state.cancel(this);
    }

    public void addObserver(MatchObserver observer) {
        observers.add(observer);
    }

    public void notifyObservers(String message) {
        log.info("[Observer] Notificando {} observer(s): {}", observers.size(), message);
        for (MatchObserver observer : observers) {
            observer.update(this, message);
        }
    }

    @PostLoad
    @PostPersist
    private void ensureStateInitialized() {
        if (this.status == null) {
            this.status = MatchStatus.CREATED;
        }
        //reconstruir el estado transient en funcion del status persistido
        if (this.status == MatchStatus.IN_PROGRESS) {
            this.state = new com.idea.authservice.api.patterns.state.InProgressState();
        } else if (this.status == MatchStatus.FINISHED) {
            this.state = new com.idea.authservice.api.patterns.state.FinishedState();
        } else if (this.status == MatchStatus.CANCELLED) {
            this.state = new com.idea.authservice.api.patterns.state.CancelledState();
        } else {
            // CREATED, ARMED, CONFIRMED -> permanecen en CreatedState hasta que se inicie
            this.state = new CreatedState();
        }
        log.info("[State] Estado transient reconstruido como {} para matchId={} (status={})",
                this.state.getClass().getSimpleName(), this.id, this.status);
    }
}
